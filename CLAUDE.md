...
- Closing a window hides it and stops its session; unplugging tears the controller down.
...
- **Stock mockup sites (pngtree etc.):** they block automated fetches, need a login to download, and their licences don't allow bundling the images into the app. Don't try to scrape them. `Assets/Frames/` has CC0 frames from Pomme Plate (github.com/ephread/PommePlate) that users import through Choose Custom Image….
